...
* The `courses/` directory contains *JCUMap* XML files for individual courses — both engineering and non-engineering courses. There are no special instructions for these files, other than to be sure to name the files correctly, as described in that directory’s `README.md` file. If you need to update any of the course mappings, simply upload the new XML files to this directory and overwrite the old files. Similarly, to add a new course to the site, simply upload its XML files, and the website will automatically recognise it.
