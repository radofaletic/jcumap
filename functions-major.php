...
function displayMajorPage($name, $shortname, $major, $program, $codeProgram, $courseCodes, $urlDisplayType, $urlPrefix, $urlScript, $accreditationDisplayScript)
{
	...
	echo '	<p class="msg-info" role="alert">Note: information provided here is indicative only. For full and current information about this major view the official page on P&amp;C.';
	if ( !isset($program) || !$program || !isset($program->name) )
	{
		echo '<br> This page depicts information about this major in a stand-alone fashion, i.e. not including the core degree it is a part of. To see the combined information of this major as part of a full degree, select one of the “<a href="#parentPrograms">parent program(s)</a>” below.';
	}
	echo '</p>' . PHP_EOL;
	...
	if ( $program && isset($program->name) )
	{
		echo '			<tr><th>program: </th><td>' . $shortname . '</td></tr>' . PHP_EOL;
	}
	else
	{
		echo '			<tr><th>major: </th><td>' . htmlspecialchars($major->name, ENT_QUOTES|ENT_HTML5) . ' major</td></tr>' . PHP_EOL;
	}
	...
	{
		echo '			<tr id="parentPrograms"><th>parent program(s): </th><td class="small" style="column-count: 2;"><ul class="noindent">' . PHP_EOL;
	...
	if ( ( isset($program->courses) && $program->courses ) || ( isset($major->courses) && $major->courses ) )
	{
		...
		{
			echo '			<tr><th>courses in core: </th><td class="small" style="column-count: 2;"><ul class="noindent">' . PHP_EOL;
		...
		echo '			<tr><th>courses in major: </th><td class="small" style="column-count: 2;"><ul class="noindent">' . PHP_EOL;
	...
	if ( $majorMappingData )
	{
		...
		foreach ($majorCompetencies as $competencyKey => $competency)
		{
			...
			{
				echo '				<tr><td colspan="' . ( 2 + $colSpan ) . '"><strong>' . $competency->label . ' ' . $competency->text . '</strong></td></tr>' . PHP_EOL;
			}
			else if ( $competency->level == 2 )
			{
				echo '				<tr class="border-bottom"><td style="width: 10%;" class="text-center bdr-right-solid" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-html="true" title="' . $competency->text . '">' . $competency->label . '</td><td colspan="' . ( 1 + $colSpan ) . '">' . PHP_EOL;
	...
	if ( $majorCompetencies )
	{
		...
		foreach ($majorCompetencies as $competencyKey => $competency)
		{
			switch ($competency->level)
			{
				...
				case 2:
					echo '				<tr';
					if ( $accreditationDisplayScript && $competency->competencyLevel > 0 )
					{
